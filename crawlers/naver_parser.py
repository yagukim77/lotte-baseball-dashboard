    ...
    while i < len(lines):
        if lines[i] == "타율":
            try:
                name = lines[i-2]
                team = lines[i-1]
                if name in ["위", "-"] or team in ["타율", "경기", "전체", "KBO리그"]:
                    i += 1
                    continue
                avg = float(lines[i+1])
                hr, rbi, ops = 0, 0, None
                for j in range(i, min(i+60, len(lines)-1)):
                    if lines[j] == "홈런":
                        ...
                        ops = float(lines[j+1]); break
                if ops is not None:
                    rows.append({"player": name, "team": team, "avg": avg, "hr": hr, "rbi": rbi, "ops": ops})
                    i += 20
                    continue
            except Exception:
                pass
    ...
    while i < len(lines):
        if lines[i] == "평균자책":
            try:
                name = lines[i-2]
                team = lines[i-1]
                if name in ["위", "-"] or team in ["평균자책", "경기", "전체", "KBO리그"]:
                    i += 1
                    continue
                era = float(lines[i+1])
                ...
                for j in range(i, min(i+50, len(lines)-1)):
                    if lines[j] == "경기":
                        ...
                        save = int(re.sub(r"[^\d]", "", lines[j+1]) or 0); break
                rows.append({"player": name, "team": team, "era": era, "game": game, "win": win, "lose": lose, "save": save})
                i += 20
                continue
            except Exception:
                pass
